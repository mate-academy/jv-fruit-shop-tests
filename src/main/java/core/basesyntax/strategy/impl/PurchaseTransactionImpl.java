...
public class PurchaseTransactionImpl implements OperationHandler {
    ...
    public void handle(String fruit, int quantity) {
        if (fruit == null) {
            throw new RuntimeException("Purchase operation failed! Incoming fruit can't be null!");
        }
        Map<String, Integer> storageMap = Storage.getFruitStore();
        if (storageMap.get(fruit) == null) {
            throw new RuntimeException("Purchase operation failed! Incoming fruit out of Storage!"
                    + " Please use Balance operation first!");
        }
        int currentStoredFruit = storageMap.get(fruit);
        if (MINIMUM_STORAGE_QUANTITY >= currentStoredFruit - quantity
                || MINIMUM_STORAGE_QUANTITY > quantity) {
            throw new RuntimeException("Purchase operation failed! Not enough " + fruit
                    + "'s in Storage!");
        }
