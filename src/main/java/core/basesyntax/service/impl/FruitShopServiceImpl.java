...
public class FruitShopServiceImpl implements FruitShopService {
    ...
    public void transfer(List<FruitTransaction> fruitTransactions) {
        checkFruitTransactions(fruitTransactions);
        for (FruitTransaction transaction : fruitTransactions) {
            transactionStrategy.get(transaction.getOperation()).processOperation(transaction);
        }
    }

    private void checkFruitTransactions(List<FruitTransaction> fruitTransactions) {
        if (fruitTransactions == null) {
            throw new NullPointerException("Empty fruit transactions");
        }
    }
}
