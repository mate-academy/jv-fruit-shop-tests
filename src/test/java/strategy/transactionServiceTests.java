...
class CsvTransactionServiceTest {
    ...
    void processCsv_WhenEmptyTransactionList_NoChangesInStorage() {
        ...
        assertEquals(0, transactionDao.getAll().size(), "Storage should remain empty if no transactions are processed");
    }

    @Test
    void processCsv_WhenValidSupplyTransaction_StorageUpdatedCorrectly() {
        FruitTransaction transaction = new FruitTransaction("banana", 20, FruitTransaction.Operation.SUPPLY);
        csvTransactionService.processCsv(List.of(transaction));
        assertEquals(20, transactionDao.getAll().get("banana"), "Storage should be updated correctly after supply transaction");
    }

    @Test
    void processCsv_WhenValidPurchaseTransaction_StorageDecreasedCorrectly() {
        FruitTransaction supplyTransaction = new FruitTransaction("orange", 30, FruitTransaction.Operation.SUPPLY);
        FruitTransaction purchaseTransaction = new FruitTransaction("orange",10,  FruitTransaction.Operation.PURCHASE);
        csvTransactionService.processCsv(List.of(supplyTransaction, purchaseTransaction));
        assertEquals(20, transactionDao.getAll().get("orange"), "Storage should decrease correctly after purchase transaction");
    }

    @Test
    void parseTransactions_WhenEmptyList_ShouldThrowException() {
        Exception exception = assertThrows(IllegalArgumentException.class, () -> csvParseService.parseTransactions(null));
