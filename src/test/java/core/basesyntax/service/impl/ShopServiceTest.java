...
public class ShopServiceTest {
    ...
    public void process_validTransactions_success() {
        ...
        FruitTransaction purchase = new FruitTransaction(FruitTransaction
                .Operation.PURCHASE, "banana", 30);

        shopService.process(Arrays.asList(balance, supply, purchase));

        assertEquals(120, ShopStorage
                .getInstance().getFruitQuantity("banana"));
    }
    ...
    public void process_returnOperation_updatesStockCorrectly() {
        FruitTransaction balance = new FruitTransaction(FruitTransaction
                .Operation.BALANCE, "banana", 10);
        FruitTransaction returnOperation = new FruitTransaction(FruitTransaction
                .Operation.RETURN, "banana", 5);

        shopService.process(Arrays.asList(balance, returnOperation));

        assertEquals(15, ShopStorage
                .getInstance().getFruitQuantity("banana"));
    }
}
