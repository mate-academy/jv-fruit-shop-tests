...
public class PurchaseOperationHandler implements OperationHandler {
    ...
    public void operate(FruitTransaction fruitTransaction) {
        ...
        if (FruitStorage.STORAGE_MAP.get(fruitType) == null) {
            throw new InvalidDataException("That is not possible to purchase "
                    + "a product that is not on the balance of the store.");
        }
        if (FruitStorage.STORAGE_MAP.get(fruitType) < fruitTransaction.getQuantity()) {
            throw new InvalidDataException("There is not enough fruit to sell in the store, "
                    + "there are " + FruitStorage.STORAGE_MAP.get(fruitType));
        }
