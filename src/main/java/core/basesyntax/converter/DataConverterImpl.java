...
public class DataConverterImpl implements DataConverter {
    ...
    private List<FruitTransaction> getSeparatedTransactions(List<String> inputData) {
        ...
        for (int i = 1; i < inputData.size(); i++) {
            ...
            if (splitted.length == SPLIT_ARRAY_LENGTH) {
                ...
                fruitTransition.setFruitName(correctFruitName(
                        splitted[FRUIT_NAME_INDEX]));
                fruitTransition.setQuantity(
                        checkForQuantityNumber(
    ...
    private String correctFruitName(String fruitName) {
        if (fruitName == null || fruitName.isEmpty() || fruitName.equals("null")) {
    ...
    private int checkForQuantityNumber(int quantity) {
        if (quantity <= 0) {
