...
public class DataValidatorServiceImpl implements DataValidatorService {
    ...
    public boolean isDataValid(List<String> data) {
        ...
        if (!data.get(HEADER_INDEX).equals(HEADER)) {
            throw new RuntimeException("Data from input file not have HEADER");
        } else {
            data.remove(HEADER_INDEX);
        }
        if (data.size() == 0) {
            throw new RuntimeException("Data from input file not have statistic");
        }
        for (int i = 0; i < data.size(); i++) {
            ...
            if (isLineLengthCorrect.length != 3
                    || !INTEGER_PATTERN
                            .matcher(isLineLengthCorrect[INDEX_FOR_PRODUCT_VALUE_IN_STRING].strip())
                            .matches()) {
                throw new RuntimeException("Data from input file not correct. "
                        + "The third group is not a positive number in line "
                        + (i + INDEX_CORRECT_FOR_LINE_IN_WHICH_ERROR));
            }
        }
        return true;
