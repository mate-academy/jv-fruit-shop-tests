...
public class DataProcessingServiceImpl implements DataProcessingService {
    ...
    public void updateDataStorage(List<FruitTransaction> fruitTransactions) {
        handleErrors(fruitTransactions);
    ...
    private void handleErrors(List<FruitTransaction> fruitTransactions) {
        if (fruitTransactions == null) {
            throw new RuntimeException("List is NULL");
        }

        if (fruitTransactions.contains(null)) {
            throw new RuntimeException("The list contains a null Object");
        }

        for (FruitTransaction fruitTransaction : fruitTransactions) {
            if (fruitTransaction.getFruit() == null || fruitTransaction.getOperation() == null) {
