...
public class ProccessDataImpl implements ProccessData {
    ...
    public void handleOperations(List<FruitTransaction> fruitTransactions,
        ...
        if (fruitTransactions.size() == 0) {
            throw new RuntimeException("FruitTransactions list mustn't be empty: "
                    + fruitTransactions);
        }
        if (mapOfHandler.size() == 0) {
            throw new RuntimeException("Map of Handlers mustn't be empty: "
                    + mapOfHandler);
        }
