...
public class InputDataServiceImpl implements InputDataService {
    ...
    private void handleErrors(String data) {
        if (data.contains("null")) {
            throw new RuntimeException("Impossible write data, field - " + data + " contains NULL");
        }

        String[] testFields = data.split(SEPARATOR);
        if (!testFields[QUALITY].matches(ONLY_NUMBERS)) {
            throw new RuntimeException("Impossible write data, Quantity case - "
                    + testFields[QUALITY] + " contains characters");
        }
    }
}
