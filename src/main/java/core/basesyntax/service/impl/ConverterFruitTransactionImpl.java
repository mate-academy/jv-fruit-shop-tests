...
public class ConverterFruitTransactionImpl implements ConverterFruitTransaction {
    ...
    @Override
    public List<FruitTransaction> convertToFruitTransaction(List<String> readFruitTransactions) {
        return readFruitTransactions.stream()
                .map(this::getFruitTransactionFromFile)
                .toList();
    }

    private FruitTransaction getFruitTransactionFromFile(String valuesFromFile) {
        String[] fruitFields = valuesFromFile.split(SEPARATOR);
