...
public class SupplyTransactionImpl implements OperationHandler {
    private static final int MINIMUM_STORAGE_QUANTITY = 0;

    @Override
    public void handle(String fruit, int quantity) {
        if (fruit == null) {
            throw new RuntimeException("Supply operation failed! Incoming fruit can't be null!");
        }
        Map<String, Integer> storageMap = Storage.getFruitStore();
        if (storageMap.get(fruit) == null) {
            throw new RuntimeException("Supply operation failed! Incoming fruit out of Storage!"
        ...
        if (MINIMUM_STORAGE_QUANTITY > currentStoredFruit || MINIMUM_STORAGE_QUANTITY > quantity) {
            throw new RuntimeException("Supply value can't be less than "
