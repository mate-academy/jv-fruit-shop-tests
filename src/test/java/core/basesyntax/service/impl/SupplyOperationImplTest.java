...
public class SupplyOperationImplTest {
    ...
    public void registerTransaction_presentFruit_Ok() {
        OperationHandler operationHandler = new SupplyOperationImpl(storageDao);
    ...
    @Test
    public void registerTransaction_nonPresentFruit_Ok() {
        OperationHandler operationHandler = new SupplyOperationImpl(storageDao);
        int expectedQty = Storage.getStorage()
                .getOrDefault(transactionOrange20.getFruit(),0) + transactionOrange20.getQuantity();
        operationHandler.registerTransaction(transactionOrange20);
        int actualQty = Storage.getStorage()
                .get(transactionOrange20.getFruit());
        assertEquals(expectedQty, actualQty);
