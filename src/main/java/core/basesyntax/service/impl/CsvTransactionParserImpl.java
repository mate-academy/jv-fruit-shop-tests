    ...
    public List<FruitTransaction> fruitTransaction(List<String> lines) {
        if (!lines.get(FIRST_LINE).equals(FIRST_LINE_FORMAT)) {
            throw new RuntimeException("Wrong data format. "
                    + "First line should be \"type,fruit,quantity\"");
        }
        ...
        for (int i = FIRST_LINE_TO_READ; i < lines.size(); i++) {
            ...
            try {
                ...
                throw new RuntimeException("Wrong data format for type(operation) entered" + e);
            } catch (NumberFormatException e) {
                throw new RuntimeException("Wrong quantity format entered. "
                       + "Quantity should be in Integer format." + e);
            }
