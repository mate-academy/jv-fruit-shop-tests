...
import core.basesyntax.service.writer.WriteToFile;
import core.basesyntax.service.writer.WriteToFileImpl;
...
public class FruitShop {
    ...
    public static void main(String[] args) {
        ...
        operationHandlersMap.put(Operation.BALANCE,
                new BalanceOperationHandlerImpl(FRUIT_SHOP_DAO));
        operationHandlersMap.put(Operation.RETURN,
                new ReturnOperationHandlerImpl(FRUIT_SHOP_DAO));
        operationHandlersMap.put(Operation.SUPPLY,
                new SupplyOperationHandlerImpl(FRUIT_SHOP_DAO));
        operationHandlersMap.put(Operation.PURCHASE,
                new PurchaseOperationHandlerImpl(FRUIT_SHOP_DAO));

        List<FruitTransaction> fruitTransactionList =
                new ReaderFileImpl().readFile(INPUT_FILE);

        OperationStrategy operationStrategy = new OperationStrategyImpl(operationHandlersMap);
        FruitTransactionToFile fruitTransactionToFile =
                new FruitTransactionToFileImpl(operationStrategy);
